...
const WorkStatusSchema = StringEnum(["planned", "active", "blocked", "completed", "cancelled"] as const);
const TodoStatusSchema = StringEnum(["pending", "active", "blocked", "completed", "cancelled"] as const);
const NameSchema = Type.String({ minLength: 1, maxLength: 240 });
const InfoSchema = Type.String({ minLength: 1, maxLength: 10_000 });

export function registerTaskTools(pi: ExtensionAPI, state: TaskState): void {
  pi.registerTool({
    name: "WorkCreate",
    label: "WorkCreate",
  ...
  pi.registerTool({
    ...
    execute(_id, params) {
      ...
      if (!work) throw new Error(`Work ${params.workId} not found.`);
  ...
  pi.registerTool({
    ...
    execute(_id, params) {
      ...
      if (!todo) throw new Error(`Todo ${params.taskId} not found.`);
...
function formatWorks(works: WorkWithTodos[]): string {
  if (works.length === 0) return "No works found in this session.";
